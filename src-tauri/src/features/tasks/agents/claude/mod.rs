...
pub struct ClaudeAgent {
    state: Arc<Mutex<ClaudeAgentState>>,
}
...
impl Default for ClaudeAgent {
    fn default() -> Self {
        Self {
            state: Arc::new(Mutex::new(ClaudeAgentState {
...
impl ClaudeAgent {
    fn args_for_worktree(worktree_path: &Path) -> anyhow::Result<Vec<String>> {
        let mut args = Vec::new();
        if let Some(session_id) = find_latest_session_id(worktree_path)? {
    ...
    fn status_from_output(&self, raw: &[u8], timestamp: Instant) -> Option<TaskStatus> {
        ...
        let status = if APPROVAL_PROMPTS
            .iter()
            .any(|prompt| screen_text.contains(prompt))
        {
...
impl Agent for ClaudeAgent {
    ...
    fn stop(&mut self) -> anyhow::Result<()> {
        ...
        .context("claude is not running")?;
        let mut child = child.lock();
        child.kill()
    }
}

impl TuiAgent for ClaudeAgent {
    fn start_tui(
        &mut self,
        worktree_path: &Path,
        ...
        let rows = rows.max(1);
        let cols = cols.max(1);
        let args = Self::args_for_worktree(worktree_path)?;
        ...
        let (master, writer, reader, child): (_, _, ReadHandle, _) = {
            ...
            let child = pair
                ...
                .context("failed to start Claude Code")?;
        ...
        std::thread::spawn(move || {
            ...
            loop {
                match reader.read(&mut buffer) {
                    ...
                    Err(error) => {
                        warn!("claude PTY read failed: {}", error);
                        break;
                    }
                }
            }
            ...
            let exit_code = loop {
                {
                    ...
                    match child_guard.try_wait() {
                        ...
                        Ok(None) => {}
                        Err(error) => {
                            warn!("claude process wait failed: {}", error);
                            break 1;
                        }
                    }
                }
    ...
    fn write(&mut self, data: &[u8]) -> anyhow::Result<()> {
        ...
        .context("claude terminal is not running")?;
